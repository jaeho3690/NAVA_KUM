...
def _state_key(annotator: str, patient_file: str) -> str:
    return f"sigh_offset::{annotator}::{patient_file}"
...
def _inject_label_button_colors() -> None:
    st.markdown(
        """
        <style>
        .st-key-sigh_label_actions div[data-testid="stColumn"]:nth-of-type(1) button {
            background-color: #16a34a !important;
            color: #ffffff !important;
            border: 1px solid #15803d !important;
        }
        .st-key-sigh_label_actions div[data-testid="stColumn"]:nth-of-type(2) button {
            background-color: #dc2626 !important;
            color: #ffffff !important;
            border: 1px solid #b91c1c !important;
        }
        .st-key-sigh_label_actions div[data-testid="stColumn"]:nth-of-type(3) button {
...
def _plot_sigh(data_df, sigh_ts, window_sec: int):
    ...
    detected_peak_all = wdf[wdf["detected_peak"] == True].copy()  # noqa: E712
    if not detected_peak_all.empty:
        detected_peak_all["y_pad"] = detected_peak_all["edi"] + pad
        fig.add_trace(
            go.Scatter(
                x=detected_peak_all["time_sec_abs"],
                y=detected_peak_all["y_pad"],
                mode="markers",
                name="detected_peak",
                marker=dict(color="#ff7f0e", size=6, symbol="triangle-up"),
            )
        )

    detected_sigh_all = wdf[wdf["detected_sigh"] == True].copy()  # noqa: E712
    if not detected_sigh_all.empty:
        fig.add_trace(
            go.Scatter(
                x=detected_sigh_all["time_sec_abs"],
                y=detected_sigh_all["edi"],
                mode="markers",
                name="detected_sigh",
    ...
    fig.add_vline(x=sigh_ts / TS_PER_SECOND, line_width=2, line_dash="dash", line_color="#d62728")
    if not wdf.empty:
...
st.set_page_config(page_title="Sigh 검증", layout="wide")
st.title("Sigh 검증")
st.caption("대상: detected_sigh 후보 지점")
...
candidates = build_sigh_candidates(data_df, patient_id)
...
labeled_ids = get_labeled_item_ids(labels_df, patient_id, "sigh")
latest_labels = get_latest_labels_map(labels_df, patient_id, "sigh")
...
st.plotly_chart(_plot_sigh(data_df, current_ts, WIDE_WINDOW_SEC), width="stretch")
st.markdown("**Zoom View (~20s)**")
st.plotly_chart(_plot_sigh(data_df, current_ts, ZOOM_WINDOW_SEC), width="stretch")

_inject_label_button_colors()
with st.container(key="sigh_label_actions"):
    ...
    if col1.button("O (Correct)", width="stretch"):
        try:
            append_label(
                annotator,
                make_label_row(annotator, patient_id, "sigh", current["item_id"], "O"),
    ...
    if col2.button("X (Incorrect)", width="stretch"):
        try:
            append_label(
                annotator,
                make_label_row(annotator, patient_id, "sigh", current["item_id"], "X"),
                version_tag=version_tag,
            )
            st.session_state[offset_key] = min(current_idx + 1, len(candidates) - 1)
            st.rerun()
    ...
    if col3.button("Not Sure", width="stretch"):
        try:
            append_label(
                annotator,
                make_label_row(annotator, patient_id, "sigh", current["item_id"], "NotSure"),
                version_tag=version_tag,
            )
            st.session_state[offset_key] = min(current_idx + 1, len(candidates) - 1)
            st.rerun()
