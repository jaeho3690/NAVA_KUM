...
def summarize_one_file(
    ...
    # Keep outputs grouped by the same version tag namespace used in labels.
    if rel_path.parts and rel_path.parts[0] == version_tag:
        out_rel_path = rel_path
    else:
        out_rel_path = Path(version_tag) / rel_path
